...
def render_efct(details, lovs):
    ...
    val = st.selectbox(f"Composition ciment ({att_id})", options, index=default_idx, key=f"efct_{att_id}")
    ...
    return {"code": "EFCT", "val": val, "val_id": str(val_id)}


def simulate_efct(bu_id, prod_ref, p_info, choice):
        ...
        if res and "f0_" in res[0]:
            ...
            for pillar in data.get('pillars', []):
                for crit in pillar.get('criteria', []):
                    if crit.get('criteriaCode') == 'EFCT':
                        note = crit.get('criteriaNote', 0)
                        break
    except Exception as e:
        st.write("Erreur parsing EFCT:", e)
