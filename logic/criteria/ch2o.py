...
def render_ch2o(details, lovs):
    st.markdown("**Émissions de formaldéhyde**")
    ...
    val = st.selectbox(f"Classe formaldéhyde ({att_id})", options, index=default_idx, key=f"ch2o_{att_id}")
    ...
    proof = st.radio("Preuve", ["Yes", "No"], index=0, horizontal=True, key="ch2o_proof")

    return {"code": "CH2O", "val": val, "val_id": str(val_id), "proof": proof}


def simulate_ch2o(bu_id, prod_ref, p_info, choice):
        ...
        if res and "f0_" in res[0]:
            ...
            for pillar in data.get('pillars', []):
                for crit in pillar.get('criteria', []):
                    if crit.get('criteriaCode') == 'CH2O':
                        note = crit.get('criteriaNote', 0)
                        break
    except Exception as e:
        st.write("Erreur parsing CH2O:", e)
